...
/*   Updated: 2018/12/17 05:27:29 by mjose            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*	if (shell->prompt.have_line && shell->prompt.line &&
	...
	return (0);
*/
	while (shell->command.list_cmd->name)
	{
		shell->command.tree = ft_memalloc(sizeof(t_tree));
		if (search_symbol(shell->command.list_cmd->name))
		{
			shell->command.tree = ft_create_tree(shell->command.list_cmd->name,
				shell->command.tree);
			exec_adv_command(&shell->environ, &shell->prompt,
				&shell->command, &shell->process);
		}
		shell->command.list_cmd = shell->command.list_cmd->next;
		free_tree(shell->command.tree);
	}
	return (0);
}
